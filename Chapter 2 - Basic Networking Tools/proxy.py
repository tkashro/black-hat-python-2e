...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if len(remote_buffer):
        print('[<==] Sending {} bytes from localhost'.format(remote_buffer))
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    try:
        server.bind((str(local_host), int(local_port)))
    except Exception as e:
        print('problem on bind: {}'.format(e))
    ...
    while True:
        ...
        line = '> Received incoming connection from {0}:{1}'.format(addr[0], addr[1])
        print(line)
